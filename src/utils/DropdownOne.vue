<template>
  ...
  >
    ...
    >
      {{ title }}
      <svg
        class="w-3 h-3 fill-current text-gray-300 cursor-pointer ml-1 shrink-0"
        viewBox="0 0 12 12"
    ...
    >
      <ul
        v-show="dropdownOpen"
        class="origin-top-right absolute top-full bg-white shadow-md ml-4 rounded-sm p-3 rounded-lg grid grid-cols-1 grid-rows-3"
      >
        <slot />
      </ul>
    </transition>
